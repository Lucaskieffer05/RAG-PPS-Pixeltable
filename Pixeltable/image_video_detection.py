...
class ImageProcessor:
    def __init__(
        self,
        directory: str = "image_pocessor_data",
    ...
    def setup(self) -> None:
        ...
        if hasattr(self.images_table, "get_info"):
            ...
            print(
                "Available indices on images_table:",
                info.get("indices", "Info method not providing index details"),
            )
    ...
    def process_images(self, image_paths: list):
        ...
        for image_path in image_paths:
            try:
                ...
                print(
                    f"Warning: Could not process image path {image_path}: {e}, skipping."
                )
    ...
    def search_image(
        ...
        if hasattr(self.images_table, "get_info"):
            info = self.images_table.get_info()
            print(
                "Available search indices on images_table:",
                info.get("indices", "No index info available"),
            )
        ...
        try:
            ...
            if search_type == "Text" and text_search_query:
                ...
                if isinstance(image_query, str):
                    ...
                    print(
                        "Error: Query de imagen inválido. Debe ser una ruta de archivo o un objeto PIL.Image."
                    )
                    return []
            else:
                print("Error: Tipo de búsqueda no soportada o consulta no proporcionada.")
                return []

            if query_input is None:
                print("Error: No hubo query.")
                return []

            sim = self.images_table.image.similarity(query_input)

            results: pxt.dataframe.DataFrameResultSet = (
                self.images_table.order_by(sim, asc=False)
                .limit(limit)
                .select(self.images_table.image, self.images_table.raw_detections)
                .collect()
            )
            return list(results)
        except Exception as e:
            print(f"Error al hacer la búsqueda de imagen: {str(e)}")
            return []
...
class VideoProcessor:
    ...
    def setup(self) -> None:
        self.videos_table = pxt.create_table(
            self.directory,
        ...
        if "audio_extract" not in self.videos_table.columns:
            self.videos_table.add_computed_column(
                audio_extract=extract_audio(self.videos_table.video, format="mp3")
            )
    ...
    def search_video(
        ...
        try:
            ...
            results = (
                self.frames_view.order_by(sim, asc=False)
                .limit(limit)
                .select(
                    self.frames_view.frame,
